...
export default {
  ...
  <div> 
    <div class="mt-3 mb-3">
      <h1 class="makecenter"> Create a New Tracker </h1>
    </div>
    <div class=" border formborder shadow-sm">
      ...
      <div class="mb-3">
        ...
        <input type="radio" id="one" value="Numerical" v-model="tracker.track_type">
        <label for="one">Numerical</label>
        &nbsp;
        <input type="radio" id="two" value="Multiple_Choice" v-model="tracker.track_type">
      ...
      <div class="mb-3">
        ...
        <div v-else-if="tracker.track_type === 'Multiple_Choice'" class="mb-3" id="mcqdiv">
          <input type='Text' placeholder="Put CSV without any spaces: A,B,C,etc" v-model="tracker.settings" class="form-control" /> 
        </div>
  ...
  methods: {
    postData() {
      FetchFunction({
        url: `${ApiUrl}/tracker`,
        init_obj: {
          method: this.$route.params.tracker_id ? 'PUT' : 'POST',
          headers: {
        ...
        .then(() => {
          alert('tracker successfully created')
